...
from datachempy import molidentfiers
from datachempy.units import insert_unit_list,fetch_unit_list, create_unit_list, unit_converter
import json
...
def setup(host=-1,user='',pw='',db=''):
    """
    Function to setup the database with the datachempy schema
    ...
    if host != -1:
        ...
        db=db_details['dbname'][0]+'_datachempy'
    else:
        ...
        all_dbs_tup=cur.fetchall()
        for i in all_dbs_tup:
            if '_datachempy' in i[0] and 'unit_list' not in i[0]:
                m=i[0]
                all_dbs.append((m[:-9],))
...
def temp_insert():
    ...
    if request.method=='POST' and 'upload_data' in request.form:
        ...
        # loop throught the CSV file, check if the smiles value is in the table, if yes, fetch the corresponding id, same goes for property, same goes for method, if it does not exist, fetch the last id and create a new entry
        ...
        if unit_flag == 'True':
            cur.execute('USE unit_list_datachempy;')
...
def molecule(dbid):
    ...
    mymol.write('xyz',app.config['UPLOAD FOLDER']+'/datachempy_{}.xyz'.format(mol_data['ID'][0]),overwrite=True)
    with open(app.config['UPLOAD FOLDER']+'/datachempy_{}.xyz'.format(mol_data['ID'][0])) as f:
        xyz = f.read()
        mol_data['XYZ'] = xyz
...
def delete(host='',user='',pw='',db=''):
    """
    Delete a database that was created using datachempy or delete data from one.
    ...
    if db !='':
        ...
        if 'dbname' in details:
            dbname=details['dbname']+'_datachempy'
        ...
        if 'submit' in details:
            ...
            if details['exampleRadios'] == 'option_null':
                ...
                return render_template('delete.html',all_dbs=all_dbs, success_msg = 'database {} deleted successfully'.format(dbname.replace('_datachempy','')))
                ...
                for k in keys:
                    ...
                    if details[k[:-3]+'_from_val']=='' and details[k[:-3]+'_to_val']=='':
                        ...
                        if from_val > to_val:
                            return render_template('delete.html',data=True, dbname=details['dbname'].replace('_datachempy',''),properties=properties,methods=methods,functionals=functionals,basis=basis_sets,forcefields=forcefields,all_dbs=all_dbs,err_msg='Minimum value for one of the properties is greater than the maximum value for it.')
                        else:
                            ...
                            con.commit()
            return render_template('delete.html',data=True, dbname=details['dbname'].replace('_datachempy',''),properties=properties,methods=methods,functionals=functionals,basis=basis_sets,forcefields=forcefields,all_dbs=all_dbs,success_msg='Deleted from database {}.'.format(details['dbname'].replace('_datachempy','')))
        else:
...
def run_config():
    ...
    app.run(debug=True)
    for i in os.listdir():
        if 'datachempy' in i and 'xyz' in i:
            os.remove(i)
    # os.rmdir('./uploads')